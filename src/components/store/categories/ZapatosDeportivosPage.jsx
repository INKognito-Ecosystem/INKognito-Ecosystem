...
const TITLE = 'Zapatos Deportivos'
const DESCRIPCION = 'Las siluetas de running y gym más buscadas del mercado. Réplica premium con amortiguación y acabados de nivel — a un precio que tiene sentido para Urabá. Despacho con Ruta del Golfo a toda la región de Urabá. Pago contraentrega.'

export async function loader() {
  return fetchCatalogCategoriaItems('store', 'Zapatos Deportivos')
}

export function meta() {
  const title = 'Zapatos deportivos | INKognito Store — Urabá'
  const description = 'Zapatillas para correr y entrenar en Chigorodó, Apartadó y toda la región de Urabá. Nike, Adidas, Asics y New Balance réplica premium. Envío con Ruta del Golfo, pago contraentrega.'
  return [
    { title },
    { name: 'description', content: description },
    { property: 'og:title', content: title },
    { property: 'og:description', content: description },
    { tagName: 'link', rel: 'canonical', href: `${import.meta.env.VITE_SITE_URL}/store/zapatos-deportivos` },
  ]
}
...
export default function ZapatosDeportivosPage() {
  ...
  const { items: catalogItems, hasMore: hayMasProductos, loading: cargandoMasProductos, loadMore: cargarMasProductos } =
    useLoadMore('store', { categoria: 'Zapatos Deportivos' }, { items: itemsIniciales, nextCursor, hasMore })
  const { prev, next } = getAdjacentCategories('zapatos-deportivos')
                ...
                {categories.map(cat => (
                  <Link
                    ...
                    className={`flex-shrink-0 text-[12px] font-extrabold pb-1 border-b-2 whitespace-nowrap ${
                      cat.link === '/store/zapatos-deportivos' ? 'text-black border-black' : 'text-black/60 border-transparent'
                    }`}
                  >
                    {cat.name}
                  </Link>
          ...
          {catalogItems.length === 0 ? (
            <div className="border border-[#C9A84C]/30 bg-white rounded-2xl p-10 text-center">
              ...
              <p className="text-gray-500 text-sm mb-6 max-w-sm mx-auto">
                Déjanos tu número y te avisamos cuando tengamos Zapatos Deportivos disponible. Sé el primero en saber.
              </p>
              <a
                href={`https://wa.me/573207911013?text=${encodeURIComponent('Hola, quiero que me avisen cuando haya Zapatos Deportivos disponible en INKognito Store.')}`}
