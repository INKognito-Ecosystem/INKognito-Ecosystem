...
export default function RopaDamaPage() {
  ...
  // Jose "cuando hago scroll en categorías y elijo una, esta vuelve a su
  // posición, dejando la selección escondida" — cada categoría de Store es
  // una página/ruta distinta, así que el listón siempre remonta con
  // scrollLeft=0, dejando la categoría activa fuera de vista si el usuario
  // la eligió más adelante en la fila).
  ...
  // useLoadMore no sirve para esto (resetea a la página inicial del server
  // en cuanto cambian los filtros, no vuelve a pedir con el término nuevo),
  // así que la búsqueda vive aparte con su propio fetch debounced.
  ...
  return (
    <>
      {/* Navbar completo — solo escritorio. En móvil lo reemplaza la barra
          compacta de abajo (logo + buscador), mismo patrón que
          SupplyCategoryPage.jsx. */}
      <div className="hidden md:block">
        <NavbarCategoryStore pageName="Ropa Dama" />
      </div>

      {/* Barra superior compacta con buscador — solo móvil (2026-09-16,
          Jose: "el buscador debe aparecer también en cada categoría... en
          el navbar superior"). */}
      ...
      {/* HERO — pt-0 en móvil (2026-09-16): el navbar de arriba ya no es
          fixed en móvil (StoreMobileNav lo reemplazó), así que ya no hace
          falta compensar su altura acá; md:pt-24 se queda para el navbar
          fixed de escritorio. */}
              ...
              <div className="flex-1 flex gap-4 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {categories.map(cat => (
                  <Link
                    key={cat.id}
                    to={cat.link}
                    ref={cat.link === '/store/ropa-dama' ? activeTabRef : null}
      ...
      <div className="bg-gray-50 pt-4 pb-8 md:pb-14 px-6">
        <div className="max-w-7xl mx-auto">
          {/* Buscador — fila propia arriba del grid, solo escritorio (en
              móvil ya vive en la barra superior compacta). Mismo estado
              `busqueda`, mismo campo. */}
