...
export default function GuayosPage() {
  ...
  const { items: catalogItems, hasMore: hayMasProductos, loading: cargandoMasProductos, loadMore: cargarMasProductos } =
    useLoadMore('store', { categoria: 'Guayos' }, { items: itemsIniciales, nextCursor, hasMore })
    ...
    <>
      ...
      <div className="relative overflow-hidden bg-gray-50 pt-20 md:pt-24">
        ...
        <div className="relative z-10 pb-4 px-6 max-w-7xl mx-auto">
                ...
                {categories.map(cat => (
                  <Link
                    ...
                    className={`flex-shrink-0 text-[12px] font-extrabold pb-1 border-b-2 whitespace-nowrap ${
                      cat.link === '/store/guayos' ? 'text-black border-black' : 'text-black/60 border-transparent'
                    }`}
                  >
                    {cat.name}
                  </Link>
          ...
          <h1 className="text-xl md:text-7xl font-black uppercase leading-none mb-2 text-gray-900 text-center md:text-left">{TITLE}</h1>
