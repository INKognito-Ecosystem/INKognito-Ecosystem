...
import EditarPerfilSupplySection from './EditarPerfilSupplySection'
import MisProductosSupplySection from './MisProductosSupplySection'
import MisVentasSupplySection from './MisVentasSupplySection'
...
const OPCIONES = [
  ...
  { key: 'productos', label: 'Mis productos en Supply', icon: ShoppingBag },
  { key: 'ventas', label: 'Mis ventas', icon: Wallet },
]

const TITULOS = { perfil: 'Editar mi perfil', productos: 'Mis productos en Supply', ventas: 'Mis ventas' }

// Panel de gestión del catálogo (2026-09-12) — mismo espíritu que
// EstudioTiendaOwnerPanel.jsx en Store, reconstruido tras el primer
// intento (que solo tenía links de salida a /estudio/mi-perfil — Jose:
// "no se ve organizado, ni tan claro como en Store"). Ahora es un menú
// real: cada opción abre su propia pantalla CON botón "← Volver", sin
// salir de este panel — "Mis productos"/"Mis ventas" reutilizan tal cual
// los componentes ya extraídos de EstudioEditarPerfilPage.jsx, "Editar mi
// perfil" es un formulario nuevo y liviano (EditarPerfilSupplySection.jsx)
// pero habla con el MISMO endpoint (PUT /api/estudios-editar-por-token)
// que ya usaba ese dashboard viejo — no hay dos fuentes de verdad.
// Mercado Pago va SIEMPRE visible en el menú (como en Store), no escondido
// detrás de un clic — es lo primero que hace falta para poder cobrar.
export default function EstudioSupplyOwnerPanel({ estudio, token, cloud_name, upload_preset, onClose, onEstudioUpdate }) {
